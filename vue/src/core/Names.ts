import {
  InMemoryNameListGateway,
  Name,
  NameListGateway
} from "@/core/gateways/names";
import { LocalStorageUsedGateway, UsedGateway } from "@/core/gateways/used";
import { LikesGateway, LocalStorageLikesGateway } from "@/core/gateways/likes";
...
export class Names {
  ...
  async load(page = 1): Promise<ViewModel> {
  ...
  static make(): Names {
    if (!Names.instance) {
      Names.instance = new Names(
        new InMemoryNameListGateway([{ uid: "1", fullName: "example name" }]),
